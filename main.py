...
from data_ingestion import get_linkedin_data, get_twitter_data, get_facebook_data
...
def ingest_data(progress_bar, company_name):
  ...
  update_progress_bar(progress_bar, 55, "Getting Facebook data...")
  facebook_data = get_facebook_data(company_name)
  update_progress_bar(progress_bar, 75, f"{len(facebook_data)} posts obtained from Facebook")
  return linkedin_data, twitter_data, facebook_data


def clean_and_transform_data(progress_bar,linkedin_data, twitter_data, facebook_data):
  ...
  update_progress_bar(progress_bar, 75, "Transforming data")
  return linkedin_data, twitter_data, facebook_data


def show_data(linkedin_data, twitter_data, facebook_data):
  st.header("Linkedin data")
  st.dataframe(linkedin_data)
  st.header("Twitter data")
  st.dataframe(twitter_data)
  st.header("Facebook data")
  st.dataframe(facebook_data)


def get_company_review(company_name):
  progress_bar = st.progress(0, text="Searching data...")
  linkedin_data, twitter_data, facebook_data = ingest_data(progress_bar, company_name)
  clean_and_transform_data(progress_bar, linkedin_data, twitter_data, facebook_data)
  update_progress_bar(progress_bar, 100, "Process finished")
  show_data(linkedin_data, twitter_data, facebook_data)
